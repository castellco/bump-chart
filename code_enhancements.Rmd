...
geom_point(size = 3, shape = 1, color="white", alpha = 0.7, stroke = 0.7) +
  scale_x_discrete(
     ...
     labels = x_names_full,
     position = "top",
     expand = expansion(mult = c(0.33, 0.1))
                  ) +
  scale_y_discrete(
    expand = expansion(mult = c(0.023, 0.022))
                  ) +
labs(
...
```{r}
ggsave("bump-enhancements5.png", plot = last_plot(), path = "C:/Users/Carolina/0_ccast/master/04_dataviz/bump-chart/bump-chart", width = 8.29, height = 6.88, units = "in", dpi = 120)
```
