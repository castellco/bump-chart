...
df <- df %>%
  ...
  mutate(dollars_arrange = sort(dollars, decreasing = FALSE))
...
# Here, I changed y=dollars to y=dollars_arrange
a <- ggplot(
  data = df,
  aes(x = generation, y = dollars_arrange, group = variables)) +
   theme_bump() +
     geom_line(aes(color = variables, alpha = 1, linewidth = dollars)) +
     geom_point(size = 11.85, color = "white") +
     geom_point(aes(color = variables), size = 11.3) +
     scale_y_reverse(waiver()) +
     scale_x_discrete(
...
```{#r}
ggsave("bump-enhancements2.png", plot = last_plot(), path = "C:/Users/Carolina/0_ccast/master/04_dataviz/bump-chart/bump-chart", width = 8.29, height = 6.88, units = "in", dpi = 120)
```