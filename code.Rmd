...
theme_bump <- function() {
  ...
  theme_bw(base_size = 15) +
    ...
    # Plot margins
    theme(plot.margin = unit(c(1, 0,2, 2, 0.1), "cm")) # First one is upper margin, second one is right margin, third is left margin
}
```
...
ggplot(
  ...
  geom_point(aes(color = variables), size = 23) +
  ...
  theme_bump() +
    geom_text(data = df %>% filter(generation == "Silent"), aes(label = variables, x = 0.9), hjust = "right", fontface = "bold", color = "black", size = 5) +
