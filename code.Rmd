...
ggplot(
  ...
  theme_bump() +
  geom_text(data = df %>% filter(generation == "Generation Z"),
            aes(label = variables, x = 1.5) , hjust = 0, fontface = "bold", color = "black", size = 4) +

  #geom_text(data =df, aes(label=dollars),hjust="center") + #FIXME
...
```{#r}
ggplot(data = df, aes(x = generation, y = ranking, group = variables)) +
  ...
  labs(x = "year",
       y = "rank",
       title = "American Kennel") +
  theme_bump() 
  # + geom_text(data =df,aes(y=variables,x=color_background,label=variables),hjust="right")
...
```{r}
ggsave("bump.png", path = "~/bump-chart", width = 10, height = 10, dpi = 300)
```
