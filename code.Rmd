...
theme_bump <- function() {
  ...
  theme_bw(base_size = 15) +
    ...
    # Format de caption
    theme(plot.caption = element_text(hjust = 0.5, size = 6)) +

    # Format title and axis labels
    theme(plot.title = element_text(color = color_text, size = 19, face = "bold", hjust = 0.5)) +
    theme(plot.subtitle = element_text(color = color_text, size = 11, hjust = 0.5, face = "bold")) +
    theme(axis.title.x = element_blank()) +
    theme(axis.title.y = element_blank()) +
    theme(axis.text = element_markdown()) +
    theme(axis.text.x = element_markdown()) +
    theme(axis.text.x.top = element_markdown(size = 8, color = "#3b3b3a")) +
    theme(axis.text.y = element_blank()) +
    # theme(strip.text = element_text(face = "bold")) +
    theme(plot.tag = element_markdown(size = 8, color = "black")) +
    theme(plot.tag.position = c(0.10, 0.855)) +
...
a <- ggplot(
  ...
  aes(x = generation, y = ranking, group = variables)) +
   theme_bump() +
     ...
     geom_point(size = 8.4, color = "white") +
     geom_point(aes(color = variables), size = 8) +
  ...
  labs(
    ...
    geom_text(data = df %>% filter(generation == "Silent"), aes(label = variables, x = 0.8), hjust = "outward", fontface = "bold", color = "#272727", size = 2.4) +
      geom_text(data = df, aes(label = dollars), hjust = "center", color = "white", size = 2.3, fontface = "bold") + # FIXME
